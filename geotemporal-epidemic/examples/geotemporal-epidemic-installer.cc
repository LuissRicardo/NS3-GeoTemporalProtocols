...
#include "geotemporal-epidemic-application.h"
...
#include <ns3/geotemporal-epidemic-helper.h>
#include <ns3/geotemporal-epidemic-packets.h>
...
// =============================================================================
//                         GeoTemporalEpidemicInstaller
// =============================================================================

NS_LOG_COMPONENT_DEFINE ("GeoTemporalEpidemicInstaller");

GeoTemporalEpidemicInstaller::GeoTemporalEpidemicInstaller () :
...
GeoTemporalEpidemicInstaller::GeoTemporalEpidemicInstaller (const GeoTemporalEpidemicInstaller& copy) :
...
bool
GeoTemporalEpidemicInstaller::Configure (int argc, char** argv)
{
  NS_LOG_FUNCTION (this);
...
void
GeoTemporalEpidemicInstaller::Run ()
{
  NS_LOG_FUNCTION (this);
...
void
GeoTemporalEpidemicInstaller::CreateNodes ()
{
  NS_LOG_FUNCTION (this);
...
void
GeoTemporalEpidemicInstaller::CreateDevices ()
{
  NS_LOG_FUNCTION (this);
  ...
  // if (m_pcap_output)
  //   {
  //     wifi_phy_helper.EnablePcapAll (std::string ("geotemporal-epidemic"), true);
  //   }
...
void
GeoTemporalEpidemicInstaller::InstallInternetStack ()
{
  NS_LOG_FUNCTION (this);

  std::cout << "Installing Internet stack... ";

  GeoTemporalEpidemicHelper epidemic_helper;

  epidemic_helper.Set ("HelloInterval", TimeValue (MilliSeconds (m_hello_packets_interval)));
  epidemic_helper.Set ("PacketsQueueCapacity", UintegerValue (m_packets_queue_capacity));
  epidemic_helper.Set ("NeighborsExpirationTime", TimeValue (Seconds (m_neighbor_expiration_time)));
  epidemic_helper.Set ("DataPacketHopsCount", UintegerValue (m_data_packet_hops_count));

  InternetStackHelper internet_stack;
  internet_stack.SetRoutingHelper (epidemic_helper); // has effect on the next Install ()
  ...
  for (uint32_t node_id = 0; node_id < m_nodes_container.GetN (); ++node_id)
    {
      m_nodes_container.Get (node_id)
              ->GetObject<geotemporal_epidemic::RoutingProtocol> ()
              ->SetNodeId (node_id);
    }
...
void
GeoTemporalEpidemicInstaller::ConfigureNodesMobility ()
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_mobility_scenario_id != "fixed")
    {
      ...
      Ptr<Node> node;
      Ptr<geotemporal_epidemic::RoutingProtocol> routing_protocol;
      ...
      for (uint32_t node_id = 0u; node_id < m_nodes_container.GetN (); ++node_id)
        {
          ...
          routing_protocol = node->GetObject<geotemporal_epidemic::RoutingProtocol> ();
...
void
GeoTemporalEpidemicInstaller::InstallAplications ()
{
  NS_LOG_FUNCTION (this);

  DestinationGeoTemporalArea dest_gta;
  Ipv4Address node_ip;
  Ptr<GeoTemporalEpidemicApplication> app;
  Ptr<Node> node;

  if (m_mobility_scenario_id == "fixed")
    {
      std::cout << "Installing Geo-Temporal Epidemic application on "
      ...
      for (uint32_t source_node_index = 0u; source_node_index < m_data_packet_source_vehicles_count;
              ++source_node_index)
        {
          ...
          app = CreateObject<GeoTemporalEpidemicApplication> ();
...
void
GeoTemporalEpidemicInstaller::FreeUnnecessaryResources ()
{
...
void
GeoTemporalEpidemicInstaller::ScheduleNextProgressReport ()
{
  NS_LOG_FUNCTION (this);
  ...
  Simulator::Schedule (Seconds (m_progress_report_time_interval),
                       &GeoTemporalEpidemicInstaller::DoProgressReport,
                       this);
}
...
void
GeoTemporalEpidemicInstaller::Report (const std::string& output_xml_filename)
{
  NS_LOG_FUNCTION (this);
