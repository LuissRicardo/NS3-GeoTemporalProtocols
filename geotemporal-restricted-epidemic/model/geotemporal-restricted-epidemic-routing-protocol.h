...
#ifndef GEOTEMPORAL_RESTRICTED_EPIDEMIC_ROUTING_PROTOCOL_H
#define GEOTEMPORAL_RESTRICTED_EPIDEMIC_ROUTING_PROTOCOL_H

#include <map>
#include <utility>
#include <vector>

#include "geotemporal-restricted-epidemic-duplicate-detector.h"
#include "geotemporal-restricted-epidemic-neighbors-table.h"
...
private:
  ...
  /**
   * UDP port used for the Geo-Temporal Restricted Epidemic routing protocol.
   */
  static const uint32_t RESTRICTED_EPIDEMIC_ROUTING_PROTOCOL_PORT = 49765;
...
public:
  ...
  /** Receives a packet that must contain a Geo-Temporal Restricted Epidemic 
   * header (HELLO, SUMMARY VECTOR, DATA) and the Type header, and returns the 
   * size of the complete packet when transmitted (Geo-Temporal Restricted 
   * Epidemic header, UDP header, IP header, IEEE 802.11 header, LCC header and 
   * frame end).
   */
  static uint32_t
...
private:
  ...
  /** 
   * Receives Restricted Epidemic packets and forwards the processing to the 
   * appropriate function.
   */
  void
  RecvRestrictedEpidemicPacket (Ptr<Socket> socket);
...
#endif /* GEOTEMPORAL_RESTRICTED_EPIDEMIC_ROUTING_PROTOCOL_H */
