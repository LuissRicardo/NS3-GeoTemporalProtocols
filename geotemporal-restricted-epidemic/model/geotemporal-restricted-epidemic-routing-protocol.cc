 ...
 * 
 * Geo-Temporal Restricted Epidemic routing protocol.
 */

#include "geotemporal-restricted-epidemic-routing-protocol.h"
...
NS_LOG_COMPONENT_DEFINE ("GeoTemporalRestrictedEpidemicRoutingProtocol");
...
{
  static TypeId tid = TypeId ("ns3::geotemporal_restricted_epidemic::RoutingProtocol")
...
{
  return packet->GetSize () // Entire Restricted Epidemic header size (including Type header)
                                   ...
                                   Ipv4InterfaceAddress new_interface_address)
{
  ...
  if (m_selected_interface_index != (int32_t) interface_index)
    {
      NS_LOG_DEBUG ("Added address doesn't belong to the interface participating "
                    "in the Geo-Temporal Restricted Epidemic operation.");
      return;
    }
  ...
  if (ipv4_l3->GetNAddresses (interface_index) > 1)
    {
      NS_LOG_DEBUG ("Added address ignored: Geo-Temporal Restricted Epidemic does NOT "
  ...
  if (m_selected_interface_index >= 0)
    {
      ...
      NS_LOG_DEBUG ("Interface ignored: Geo-Temporal Restricted Epidemic does NOT "
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvRestrictedEpidemicPacket, this));
  ...
  socket->Bind (InetSocketAddress (m_selected_interface_address.GetLocal (),
                                   RESTRICTED_EPIDEMIC_ROUTING_PROTOCOL_PORT));
                                      ...
                                      Ipv4InterfaceAddress removed_interface_address)
{
  ...
  if (m_selected_interface_address != removed_interface_address)
    {
      NS_LOG_DEBUG ("Removed address is not participating in the "
                    "Geo-Temporal Restricted Epidemic operation.");
      return;
    }
  ...
  if (!m_unicast_socket)
    {
      NS_LOG_LOGIC ("No Geo-Temporal Restricted Epidemic interfaces.");
      Stop ();
    }
}
...
RoutingProtocol::NotifyInterfaceUp (uint32_t interface_index)
{
  ...
  if (ipv4_l3->GetNAddresses (interface_index) > 1)
    {
      NS_LOG_WARN ("Extra addresses ignored: Geo-Temporal Restricted Epidemic does NOT "
                   "work with more than one address per each interface.");
    }
...
RoutingProtocol::NotifyInterfaceDown (uint32_t interface_index)
{
  ...
  NS_LOG_LOGIC ("Geo-Temporal Restricted Epidemic interface disabled.");
}
                             ...
                             ErrorCallback error_cb)
{
  ...
  if (destination_ip.IsMulticast ())
    {
      // Geo-Temporal Restricted Epidemic does not support multicasting.
      return false;
    }
                              ...
                              Ptr<NetDevice> output_device, Socket::SocketErrno& sockerr)
{
  ...
  if (m_selected_interface_index == -1)
    {
      NS_LOG_DEBUG ("No Geo-Temporal Restricted Epidemic interfaces.");
...
void
RoutingProtocol::RecvRestrictedEpidemicPacket (Ptr<Socket> socket)
{
                             ...
                             const GeoTemporalArea& destination_gta)
{
  ...
  const DataHeader data_packet (/*Data ID*/ data_id,
                                /*Creation coordinates*/ my_position,
