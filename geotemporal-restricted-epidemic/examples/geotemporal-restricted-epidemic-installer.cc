...
#include "geotemporal-restricted-epidemic-application.h"
...
#include <ns3/geotemporal-restricted-epidemic-helper.h>
#include <ns3/geotemporal-restricted-epidemic-packets.h>
#include <ns3/geotemporal-restricted-epidemic-routing-protocol.h>
...
// =============================================================================
//                    GeoTemporalRestrictedEpidemicInstaller
// =============================================================================

NS_LOG_COMPONENT_DEFINE ("GeoTemporalRestrictedEpidemicInstaller");

GeoTemporalRestrictedEpidemicInstaller::GeoTemporalRestrictedEpidemicInstaller () :
...
m_neighbor_expiration_time (10u), m_data_packet_hops_count (8u),
...
GeoTemporalRestrictedEpidemicInstaller::GeoTemporalRestrictedEpidemicInstaller (const GeoTemporalRestrictedEpidemicInstaller& copy) :
...
m_data_packet_hops_count (copy.m_data_packet_hops_count),
...
bool
GeoTemporalRestrictedEpidemicInstaller::Configure (int argc, char** argv)
{
  NS_LOG_FUNCTION (this);
  ...
  cmd.AddValue ("dataPacketHopsCount",
                "The number of hops that each DATA packet is allowed to travel "
                "before being discarded. "
                "[Default value: 8]",
                m_data_packet_hops_count);
...
void
GeoTemporalRestrictedEpidemicInstaller::Run ()
{
  NS_LOG_FUNCTION (this);
  ...
  std::cout << " - Data packet hops count          :  " << m_data_packet_hops_count << " hops\n";
  std::cout << "\n";
...
void
GeoTemporalRestrictedEpidemicInstaller::CreateNodes ()
{
  NS_LOG_FUNCTION (this);
...
void
GeoTemporalRestrictedEpidemicInstaller::CreateDevices ()
{
  NS_LOG_FUNCTION (this);
  ...
  // if (m_pcap_output)
  //   {
  //     wifi_phy_helper.EnablePcapAll (std::string ("geotemporal-restricted-epidemic"), true);
  //   }
...
void
GeoTemporalRestrictedEpidemicInstaller::InstallInternetStack ()
{
  NS_LOG_FUNCTION (this);

  std::cout << "Installing Internet stack... ";

  GeoTemporalRestrictedEpidemicHelper restricted_epidemic_helper;

  restricted_epidemic_helper.Set ("HelloInterval", TimeValue (MilliSeconds (m_hello_packets_interval)));
  restricted_epidemic_helper.Set ("PacketsQueueCapacity", UintegerValue (m_packets_queue_capacity));
  restricted_epidemic_helper.Set ("NeighborsExpirationTime", TimeValue (Seconds (m_neighbor_expiration_time)));
  restricted_epidemic_helper.Set ("DataPacketHopsCount", UintegerValue (m_data_packet_hops_count));

  InternetStackHelper internet_stack;
  internet_stack.SetRoutingHelper (restricted_epidemic_helper); // has effect on the next Install ()
  ...
  for (uint32_t node_id = 0; node_id < m_nodes_container.GetN (); ++node_id)
    {
      m_nodes_container.Get (node_id)
              ->GetObject<geotemporal_restricted_epidemic::RoutingProtocol> ()
              ->SetNodeId (node_id);
    }
...
void
GeoTemporalRestrictedEpidemicInstaller::ConfigureNodesMobility ()
{
  NS_LOG_FUNCTION (this);
  ...
  if (m_mobility_scenario_id != "fixed")
    {
      ...
      Ptr<Node> node;
      Ptr<geotemporal_restricted_epidemic::RoutingProtocol> routing_protocol;
      ...
      for (uint32_t node_id = 0u; node_id < m_nodes_container.GetN (); ++node_id)
        {
          ...
          routing_protocol = node->GetObject<geotemporal_restricted_epidemic::RoutingProtocol> ();
              ...
              if (node_initial_time < m_simulation_duration)
                {
                  Simulator::ScheduleWithContext (node_id, Seconds (node_initial_time),
                                                  &geotemporal_restricted_epidemic::RoutingProtocol::Enable,
                                                  routing_protocol);
                }
            }
          ...
          if (node_end_time < m_simulation_duration)
            {
              Simulator::ScheduleWithContext (node_id, Seconds (node_end_time),
                                              &geotemporal_restricted_epidemic::RoutingProtocol::Disable,
                                              routing_protocol);
            }
        }
...
void
GeoTemporalRestrictedEpidemicInstaller::InstallAplications ()
{
  NS_LOG_FUNCTION (this);

  DestinationGeoTemporalArea dest_gta;
  Ipv4Address node_ip;
  Ptr<GeoTemporalRestrictedEpidemicApplication> app;
  Ptr<Node> node;

  if (m_mobility_scenario_id == "fixed")
    {
      std::cout << "Installing Geo-Temporal Restricted Epidemic application on "
      ...
      for (uint32_t source_node_index = 0u; source_node_index < m_data_packet_source_vehicles_count;
              ++source_node_index)
        {
          ...
          app = CreateObject<GeoTemporalRestrictedEpidemicApplication> ();
...
void
GeoTemporalRestrictedEpidemicInstaller::FreeUnnecessaryResources ()
{
  ...
  // We don't need the GPS system anymore in the simulation for the Restricted 
...
void
GeoTemporalRestrictedEpidemicInstaller::ScheduleNextProgressReport ()
{
  NS_LOG_FUNCTION (this);
  ...
  Simulator::Schedule (Seconds (m_progress_report_time_interval),
                       &GeoTemporalRestrictedEpidemicInstaller::DoProgressReport,
                       this);
}
...
void
GeoTemporalRestrictedEpidemicInstaller::Report (const std::string& output_xml_filename)
{
  NS_LOG_FUNCTION (this);
  ...
  for (uint32_t node_id = 0u; node_id < m_nodes_container.GetN (); ++node_id)
    {
      ...
      const std::vector<std::pair<geotemporal_restricted_epidemic::DataHeader, Time> > & created_packets
              = routing_protocol->GetCreatedDataPackets ();

      for (std::vector<std::pair<geotemporal_restricted_epidemic::DataHeader, Time> >::const_iterator
